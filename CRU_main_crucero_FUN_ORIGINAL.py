...
def cruise_sim(profile,N, otp, mod_aeronave, **kwargs):
    ...
    for j in range(0,M):
        ...
        if d_h[j] == 0:            
            ...
            t_recta[j] = Drg[j]/Thr_disp[j]
            ct[j] = miavion._fflow_model_(Thr_disp[j]*t_recta[j]/funcs.OPEN_2_EN['force'], h_prof[j], ct_factor = funcs.OPEN_2_EN['mass'])
        ...
        #Caso con cambio de altitud
            ...
            P_use[j] = Thr[j]*Va_prof[j] #Potencia utilizada
            t_recta[j] = Drg[j]/Thr_disp[j]
            ct[j] = miavion._fflow_model_(Thr_disp[j]*t_recta[j]/funcs.OPEN_2_EN['force'], h_prof[j+1], ct_factor = funcs.OPEN_2_EN['mass'])
            ct[j] = ct[j]/Drg[j] #OJO: CT LB / S PERO EQS CONSIDERAN LB / LBF S. Además aplico D = T
            P_req[j] = Drg[j]*Va_prof[j]
            q_1[j] = 0.5*rho[j]*Va_prof[j]**2
            r_1[j] = k/(np.power(q_1[j]*S,2)*CD0[j])
...
if __name__ == "__main__":
    ...
    wind_model = krg.exp_imp_modelo(0,'imp',{'folder':'wind_models', 'name':'full_BR_AR_WS'})
    wind_modelWD = krg.exp_imp_modelo(0,'imp',{'folder':'wind_models', 'name':'full_BR_AR_WD'})

...
def generar_input(lista_nombres,n_dots,data_req,**kwargs):
    ...
    for a in range(len(lista_nombres)):
        ...
        file.name = lista_nombres[a]
        
        loc_h,loc_ws, loc_wd, loc_lat, loc_lon = gen.lector_txt_fixed_size(file,n_dots)
...
#Cargar un modelo previo y devolverlo para su lectura
def cargar(ruta, name):
    print("Carga por datos")
    loaded_model = exp_imp_meta(0,0,ruta,name)
    return(loaded_model)
...
def analisis_PUNTUAL_1D(h_data, y_data, scale):
    start = time.time()
...
def exp_imp_meta(analisis,modo,ruta, nombre):
    '''MODO 1 para exportar análisis bajo nombre'''
    
    study = ot.Study()
    fileName = ruta+"/"+nombre+".xml"
